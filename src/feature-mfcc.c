...
static void knf_compute_dct(int32_t rows, int32_t cols, float *out) {
  if (rows <= 0 || cols <= 0 || out == nullptr) {
    return;
  }

  float norm0 = sqrtf(1.0f / cols);
  for (int32_t i = 0; i < cols; ++i) out[i] = norm0;
...
static void knf_compute_lifter(float lifter, int32_t num_ceps, float *out) {
  if (lifter == 0.0f || num_ceps <= 0 || out == nullptr) {
    return;
  }
...
[[nodiscard]] bool knf_mfcc_computer_create(const knf_mfcc_opts *opts,
  ...
  if (opts == nullptr || out == nullptr || opts->num_ceps <= 0 ||
      opts->mel_opts.num_bins <= 0) {
    return false;
  }
  ...
  if ((size_t)opts->num_ceps > SIZE_MAX / (size_t)opts->mel_opts.num_bins) {
    knf_mfcc_computer_destroy(out);
    return false;
  }
  out->dct_matrix = (float *)calloc(
      (size_t)opts->num_ceps * (size_t)opts->mel_opts.num_bins, sizeof(float));
...
void knf_mfcc_compute(knf_mfcc_computer *c, float signal_raw_log_energy,
  ...
  if (c == nullptr || signal_frame == nullptr || feature == nullptr ||
      ...
      c->mel_energies == nullptr || c->dct_matrix == nullptr) {
    return;
  }
